...
;-------------------------------------------------------------------------------------------
;|                  lo1s >  |  empty                    | (cons 1String ListOf1String)      |        
;|  pattern                 |                           |                                   |
;|   V                      |                           |                                   |
;|--------------------------|---------------------------|-----------------------------------|
;|  empty                   |         empty? pattern    |           empty? pattern          |
;|                          |             true          |                true               |
;|--------------------------|---------------------------|-----------------------------------|
;|     (cons "A" Pattern)   |         empty? lo1s       | (and <string=? A first pattern>   |
;|                          |            false          |      <alphabetic? first lo1s>     |
;|--------------------------|---------------------------------------------------------------- 
;|     (cons "N" Pattern)   |         empty? lo1s       | (and <string=? N first pattern>   |
;|                          |            false          |      <numeric? first lo1s>        |
;|-------------------------------------------------------------------------------------------

;; Pattern ListOf1String -> Boolean
;; produce true if pattern matches the list of 1 string
;; If the ListOf1String is longer than the pattern, but the first part matches the whole pattern produce true
;; If the ListOf1String is shorter than the Pattern you should produce false.
...
(check-expect (pattern-match? empty (list "1")) true)
(check-expect (pattern-match? (list "A") (list "A")) true)
(check-expect (pattern-match? (list "N") (list "1" "A")) true)
(check-expect (pattern-match? (list "N" "A") (list "1" "A")) true)
(check-expect (pattern-match? (list "N" "A") (list "1" "A" "1")) true)
(check-expect (pattern-match? (list "N") (list "A")) false)
(check-expect (pattern-match? (list "A") (list "1")) false)
(check-expect (pattern-match? (list "N" "A") (list "1")) false)
(check-expect (pattern-match? (list "N" "A" "A") (list "1" "A")) false)

(define (pattern-match? pattern lo1s)
  (cond [(empty? pattern) true]
        [(empty? lo1s) false]
        [(or (and (string=? "A" (first pattern))
              (alphabetic? (first lo1s)))
             (and (string=? "N" (first pattern))
              (numeric? (first lo1s))))
         (pattern-match? (rest pattern) (rest lo1s))]
        [else
         false]))
